...
LOCAL_CONVERSATIONS_BACKUP_DIR="/Users/aracho/Dropbox/BACKUP/claude_conversations/tars_aracho"
...
usage() {
    cat <<EOF
Usage:
  $(basename "$0")                 Back up remote home to Dropbox
...
# Ensure a live SSH ControlMaster connection (password prompted once).
ensure_ssh() {
    ...
    if [ -S "$SOCKET_PATH" ]; then
        ...
        echo "No SSH socket found. Connecting to $REMOTE_HOST..."
        ssh -MNf \
            -o ControlPath="$SOCKET_PATH" \
            -o ControlPersist=10m \
            "$REMOTE_USER@$REMOTE_HOST"
...
# Default backup: home to Dropbox.
backup_default() {
